...
describe('menu classification', () => {
  ...
  it('uses alcoholCheck field', () => {
    expect(isAlcoholMenuItem(item({ alcoholCheck: 1 }))).toBe(true)
    expect(isAlcoholMenuItem(item({ alcoholCheck: 0 }))).toBe(false)
    expect(isAlcoholMenuItem(item({ name: '生ビール', alcoholCheck: 1 }))).toBe(true)
    expect(isAlcoholMenuItem(item({ name: 'ノンアルコールビール', alcoholCheck: 0 }))).toBe(false)
    expect(isAlcoholMenuItem(item({}))).toBe(false)
  })
})
