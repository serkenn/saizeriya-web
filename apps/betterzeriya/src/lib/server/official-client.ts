import { createClient, type AccountSummary, type ClientState, type LookupItemResult } from '@repo/client';
...
export const createOfficialSession = async (qrURLSource: string) => {
	...
	return { id, state: client.getState(), officialSession: createSnapshot(id, client, cookieFetch.getCookies(), now) };
};
...
export const parseOfficialSessionSnapshot = (value: unknown): OfficialSessionSnapshot | undefined => {
...
export const showOfficialReceipt = async (
	...
	return {
		...
		barcodeValue:
			receiptResult.receipt.barcodeValue ?? createCheckoutCode(receiptResult.state, accountResult.account),
